...
#         #concine classes from class_negative into 1 string separated by ', '
#         class_negative = ', '.join(class_negative)
        
#         #compute scores for all classes in class_list
#         class_positive = ', '.join(class_list)
...
#         scores = []
#         text_features_pos = model.encode_text(open_clip.tokenize(class_positive).to(device))
#         text_features_pos /= text_features_pos.norm(dim=-1, keepdim=True)
#         scores_pos = (100.0 * image_features @ text_features_pos.T).softmax(dim=-1)
#         scores.append(scores_pos.cpu().numpy()[0][0])
#         ground_truth.append(1)

#         text_features_neg = model.encode_text(open_clip.tokenize(class_negative).to(device))
#         text_features_neg /= text_features_neg.norm(dim=-1, keepdim=True)
#         scores_neg = (100.0 * image_features @ text_features_neg.T).softmax(dim=-1)
#         scores.append(scores_neg.cpu().numpy()[0][0])
